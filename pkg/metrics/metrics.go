...
func New(log *logrus.Entry) *Metrics {
	containerImageVersion := promauto.NewGaugeVec(
		...
		[]string{
			"namespace", "pod", "container", "container_type", "image", "current_version", "latest_version", "last_updated", "priority",
		},
	)
...
func (m *Metrics) AddImage(namespace, pod, container, containerType, imageURL string, isLatest bool, currentVersion, latestVersion, lastUpdated, priority string) {
	...
	m.containerImageVersion.With(
		m.buildLabels(namespace, pod, container, containerType, imageURL, currentVersion, latestVersion, lastUpdated, priority),
	).Set(isLatestF)
...
func (m *Metrics) buildLabels(namespace, pod, container, containerType, imageURL, currentVersion, latestVersion, lastUpdated, priority string) prometheus.Labels {
